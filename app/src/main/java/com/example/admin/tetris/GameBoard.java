...
public class GameBoard {
    ...
    private final int[][] gameBoard = new int[boardHeight][boardWidth];
    ...
    public int codeToColor(int x, int y) {
        ...
        if (gameBoard[x][y] == 0) return Color.parseColor("#fa82e4");  // Yellow
    ...
    public boolean piece_Can_Move(Piece currentPiece, int x, int y) {
        int tmp = 0;
        /*
        ...
        ArrayList<Point> tmpPieceCoordinates = new ArrayList<>();
    ...
    private boolean can_Move_Left(Piece currentPiece) {
        return piece_Can_Move(currentPiece, 0, -1);
    }
    ...
    public boolean can_Move_Down(Piece currentPiece) {
        return piece_Can_Move(currentPiece, 1, 0);
    }
    ...
    public void rotatePiece(Piece currentPiece) {
        ...
        if (piece_Can_Rotate(currentPiece) && currentPiece.colorCode != 1) {
            deletePiece(currentPiece);
    ...
    public int clearRows() {
        ...
        if (deletedRows >= 1) {
            ...
            for (int i = 0; i < gameBoardCopy.length; i++) {
                System.arraycopy(gameBoard[i], 0, gameBoardCopy[i], 0, gameBoardCopy[1].length);
            }

            for (int i = 0; i < gameBoardCopy.length; i++) {
                System.arraycopy(gameBoardCopy[i], 0, gameBoard[i + deletedRows], 0, gameBoardCopy[1].length);
            }
        }
        return deletedRows;
    }
    ...
    public boolean checkGameOver(Piece spielStein) {
        return !can_Move_Down(spielStein) && spielStein.getMinXCoordinate(
